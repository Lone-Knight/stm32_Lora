...
rt_inline int _serial_dma_rx(struct rt_serial_device *serial, rt_uint8_t *data, int length)
{
    ...
    if (serial->config.bufsz == 0)
    {
        ...
        if (rx_dma->activated != RT_TRUE)//reinit  rx dma
        {
    ...
    else
    {
        ...
        if (rx_fifo->get_index + recv_len < serial->config.bufsz)
            rt_memcpy(data, rx_fifo->buffer + rx_fifo->get_index, recv_len);// copy direct
        else
        {
            rt_memcpy(data, rx_fifo->buffer + rx_fifo->get_index,//copy in segment bitween  get_index and  tail
                    serial->config.bufsz - rx_fifo->get_index);
            rt_memcpy(data + serial->config.bufsz - rx_fifo->get_index, rx_fifo->buffer, //and  copy the rest data
 ...
 */
static rt_err_t rt_serial_init(struct rt_device *dev)//配置初始化，在第一次Open时候，会调用  not config workmode of tx/rx
{
    ...
    /* apply configuration */
    if (serial->ops->configure)//Uart setting parameters
        result = serial->ops->configure(serial, &serial->config);

    return result;
}

static rt_err_t rt_serial_open(struct rt_device *dev, rt_uint16_t oflag)//assign work mode
{
...
void rt_hw_serial_isr(struct rt_serial_device *serial, unsigned long event)
{
    switch (event & 0xff)
    {
        ...
        case RT_SERIAL_EVENT_TX_DMADONE:
        {
            ...
            {
                serial->parent.tx_complete(&serial->parent, (void*)last_data_ptr);//give the tx data ptr,recovery memery
            }
            break;
        }
