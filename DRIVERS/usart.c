...
static void DMA_Configuration(struct rt_serial_device *serial,int bRxorTx)//增加参数int bRxorTx
	{
    struct stm32_uart *uart = (struct stm32_uart *) serial->parent.user_data;
    ...
    /* tx dma config */
		...
		uart->tx_dma.setting_data_len = 0;//发送时设置
    DMA_DeInit(uart->tx_dma.tx_ch);
